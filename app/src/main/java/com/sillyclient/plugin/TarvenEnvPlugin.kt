...
class TarvenEnvPlugin : Plugin() {
    ...
    /** 检查 SillyClient 主仓库的最新正式版本。 */
    @PluginMethod
    fun checkAppUpdate(call: PluginCall) {
        Thread {
            try {
                val packageInfo = context.packageManager.getPackageInfo(context.packageName, 0)
                val currentVersion = packageInfo.versionName ?: "unknown"
                val apiUrl = "https://api.github.com/repos/CAPTCHAAAAA/SillyClient/releases/latest"
                var release: JSONObject? = null
                var lastError: Exception? = null
                listOf(apiUrl, "https://gh-proxy.com/$apiUrl").forEachIndexed { index, candidate ->
                    if (release != null) return@forEachIndexed
                    var conn: HttpURLConnection? = null
                    try {
                        conn = URL(candidate).openConnection() as HttpURLConnection
                        conn.connectTimeout = if (index == 0) 8000 else 15000
                        conn.readTimeout = if (index == 0) 12000 else 18000
                        conn.setRequestProperty("Accept", "application/vnd.github+json")
                        conn.setRequestProperty("User-Agent", "SillyClient")
                        conn.connect()
                        ...
                        release = JSONObject(conn.inputStream.bufferedReader().use { it.readText() })
                    } catch (error: Exception) {
                        lastError = error
                    } finally {
                        conn?.disconnect()
                ...
                var latestVersion = release?.optString("tag_name", "")?.removePrefix("v")?.trim().orEmpty()
                var releaseUrl = release?.optString("html_url", "").orEmpty()
                var publishedAt = release?.optString("published_at", "").orEmpty()
                if (latestVersion.isBlank()) {
                    var conn: HttpURLConnection? = null
                    try {
                        conn = URL("https://data.jsdelivr.com/v1/package/gh/CAPTCHAAAAA/SillyClient")
                        ...
                        conn.connect()
                        if (conn.responseCode != HttpURLConnection.HTTP_OK) {
                            throw IllegalStateException("jsDelivr HTTP ${conn.responseCode}")
                        }
                        val metadata = JSONObject(conn.inputStream.bufferedReader().use { it.readText() })
                        val versions = metadata.optJSONArray("versions")
                        latestVersion = if (versions != null && versions.length() > 0) {
                            versions.optString(0, "").removePrefix("v").trim()
                        } else ""
                        releaseUrl = "https://github.com/CAPTCHAAAAA/SillyClient/releases/latest"
                        publishedAt = ""
                    } catch (error: Exception) {
                        throw IllegalStateException(error.message ?: lastError?.message ?: "网络请求失败")
                    } finally {
                        conn?.disconnect()
                    }
                }
                if (latestVersion.isBlank()) throw IllegalStateException("未找到最新版本")
                val result = JSObject()
                result.put("currentVersion", currentVersion)
                result.put("latestVersion", latestVersion)
                result.put("updateAvailable", compareVersions(currentVersion, latestVersion) < 0)
                result.put("releaseUrl", releaseUrl)
                result.put("publishedAt", publishedAt)
                call.resolve(result)
            } catch (error: Exception) {
                call.reject("checkAppUpdate: ${error.message}")
            }
        }.start()
    }
